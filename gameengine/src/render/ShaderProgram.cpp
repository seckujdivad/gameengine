...
void ShaderProgram::Select(int texture_group_id)
{
	...
	else
	{
		...
		if ((static_cast<int>(textures.size()) + SHADERPROGRAM_RESERVED_TEXTURE_UNITS) > this->m_max_texture_units)
		{
		...
		{
			glActiveTexture(GL_TEXTURE0 + SHADERPROGRAM_RESERVED_TEXTURE_UNITS + i);
			textures.at(i).Bind();
...
{
	glUniform2iv(this->GetUniform(name), 1, glm::value_ptr(vec));
}
...
{
	glUniform3iv(this->GetUniform(name), 1, glm::value_ptr(vec));
}
...
{
	glUniform4iv(this->GetUniform(name), 1, glm::value_ptr(vec));
}
